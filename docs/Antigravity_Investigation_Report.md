...
## 10. Antigravity を LLM サービス（MCP）として利用するための仕様（現状は困難）

Antigravity 自身を MCP サーバーのバックエンドとして振る舞わせること（外部からのリクエストを Antigravity で処理し、LLM の純粋な推論結果だけを結果として返すこと）は、**現在のアーキテクチャおよび拡張機能 API の制限上、極めて困難（実質的に不可能）**であることが判明しました。

以下に、以前想定されていた理想的なフローと、それが機能しない現実的な理由（ブロッカー）をまとめます。

### 想定されていた理想のフロー（現状不可）

1. **リクエスト送信**: 外部から `antigravity.sendTextToChat` に `{ request_type: "silent", modelId: ... }` などの特殊なペイロードを渡して推論を開始する。
2. **サイレント実行**: パネル等には出力させず、`agentIdSilent` などの内部フラグを用いてバックエンドに直接処理させる。
3. **結果のコールバック受信**: `ChatModel` などの内部クラスの `onDidReceiveResponse` イベントエミッタを購読し、ストリーミングで `deltaText` を取得する。

### なぜ不可能・困難なのか（現在のブロッカー）

1. **入力 API の欠如（コマンド制約）**:
   前項で述べた通り、VS Code のコマンドパレット（外部 API）から呼び出せる `antigravity.sendTextToChat` は、入力を単一のプロンプト文字列として強制キャストしてしまいます。サイレント実行フラグやモデルIDを指定したリクエストを外部から流し込む「口」が塞がれています。
2. **出力イベント（イベントエミッタ）へのアクセス不可**:
   推論結果を受け取るための `onDidAcceptResponse` などのイベントや `ChatModel` に関連するクラスは、IDE のメインプロセスまたは拡張機能ホスト内の**内部スコープ（Private / Internal）**で作動しています。サードパーティの拡張機能からこれらのイベントにフックし、コールバック関数を登録する標準的な API は提供されていません。
3. **gRPC パス（`ApiServerService` 等）への直接通信の遮断**:
   UI を介さずに直接バックエンドの Go サーバーと ConnectRPC で通信できれば MCP 化は可能ですが、ローカルサーバーとの通信には起動時にランダム生成される証明書（`cert.pem`）や、一過性の内部認証トークン、SSO プライベートプロキシを経由する厳格な制約が設けられています。外部スクリプトからこのセッションを乗っ取って通信を再現することは現実的ではありません。

### 結論
Antigravity を「外部から叩ける単なる LLM ファクトリー（LLM as a Service）」として直接利用・抽出するアプローチは、現在のセキュリティ機構および API 制約では実現できません。外部連携を行う場合は「CLI のターミナル出力を介した非同期な連携（ファイル経由での連携）」など、より限定的な運用にとどめる必要があります。
