...
そのため、現在の Antigravity IDE においては、**「VS Code API の外部コマンド（`executeCommand`）経由で、プロンプトテキストとモデル ID を同時にプログラムから指定して投げ込む設計（ルート）は意図的に用意されていない（あるいは未実装）」**という結論になります。モデル選択は IDE のパネル上で手動で行う必要があり、外部リクエストやコマンドからモデル ID を直接指定して送信することはできません。
...
## 10. Antigravity を LLM サービス（MCP）として利用するための仕様と設計方針

Antigravity IDE が持つ強力なLLM資源（Gemini 3.1 Pro等）を外部から MCP サーバーとして利用するためのアプローチについて、技術的調査を踏まえた実現方法とリスクをまとめます。

### A. gRPC通信の直接ハイジャック（警告：非推奨・BANリスク極大）

Language Server (Goバイナリ) の起動引数を確認したところ、IDEとの通信に必要な認証情報（`--csrf_token`, クライアント連携ポート等）がOSのプロセス情報（`ps`等）に平文で露出していることが判明しました。

- **技術的ハック:** この仕様を突けば、OSレベルでプロセスをスキャンするか、Language Serverバイナリをラッパースクリプトに差し替えることで、動的生成される証明書（`cert.pem`）とトークンを窃取可能です。これにより、自前で立てたプロキシサーバーから直接 `ApiServerService` のgRPCエンドポイントを叩き、Antigravityを純粋なLLM APIとして振る舞わせること（ストリーミング応答やモデル指定を含む）が**技術的には可能**です。
- **致命的なリスク:** この手法は、**Googleの利用規約（TOS）違反やアカウントBANのリスクが極めて高い**ため絶対に推奨されません。UIを通さない不自然なAPIコールは、正規のエディタ操作（テレメトリやメタデータ通信）とパターンの不一致を生み、セキュリティ監視システム（不正利用検知）に即座にキャッチされます。

### B. エージェント連携アーキテクチャ（推奨：安全な Ghost Editor 方式）

BANのリスクをゼロに抑えつつ、規約に完全に準拠してAntigravityをMCPのLLMリソースとして利用するには、**「IDEの正規の利用フローを通じた間接的な制御（ファイルI/Oベースの非同期連携）」**を採用する必要があります。

#### 連携のフロー（Ghost Editor パターン）

外部プログラムからAPIを直接ハックするのではなく、「**IDE上で動くAIエージェントに、テキストファイルを通じて仕事を依頼し、結果を別のファイルに書き込ませて回収する**」という仕組みを構築します。

1. **ブリッジ拡張機能の導入**: Antigravity IDE内に、自作の「MCPブリッジ拡張機能（ローカルサーバー）」をインストールし、バックグラウンドで起動させておきます。
2. **リクエストの中継**: 外部（MCPクライアント）からリクエストが来ると、ブリッジ拡張機能はそのプロンプトを一時ファイル（例: `/tmp/agy_input.md`）に保存します。
3. **正規コマンドの発行**: ブリッジ拡張機能は、VS Codeの正規APIを用いてチャットコマンドを内部発行します。
   - `vscode.commands.executeCommand('antigravity.sendTextToChat', 'Please strictly process the instructions in /tmp/agy_input.md and save the final response ONLY to /tmp/agy_output.md without chatting.')`
4. **結果の待機と回収**: ブリッジ拡張機能はファイルシステムイベント（`fs.watch`）で `/tmp/agy_output.md` が作成・更新されるのを待ちます。書き込みが完了次第、内容を読み取って外部のMCPクライアントにHTTP/stdioレスポンスとして返却します。

#### メリットとデメリット（トレードオフ）

- **メリット（安全性）**: 認証トークンや内部gRPCは一切触りません。すべてがLanguage Serverの監視下で「正規ユーザーの活発な利用」として処理されるため、**アカウントBANのリスクを完全に排除**できます。また、ブラウザ操作（`BrowserSubagent`）やターミナルコマンド実行（`RunCommand`）など、IDEに統合された強力なツール能力もそのまま結果に反映させることができます。
- **デメリット（制約）**: 
  - トークン単位でのリアルタイムなストリーミング（SSE出力）を外部へ中継することは困難であり、バッチ処理（完了後の結果一括返却）となります。
  - **動的なモデル変更の制限**: `antigravity.sendPromptToAgentPanel` では外部からモデルIDを引数で渡してもUIに反映されません。調査の結果、モデル設定は `~/.config/Antigravity/User/globalStorage/state.vscdb` にProtobuf形式で保存されていることが判明しました。これをプログラムから強制的に書き換えること（DBパッチ）は可能ですが、IDEのWebviewは起動時にDBを読み込んだ後はオンメモリのRedux Stateでモデルを管理しているため、再起動（Reload Window）なしには即時に反映されません。したがって、実行には「現在UI上で選択されているモデル」が利用されます。

結論として、Antigravity を外部から利用可能な LLM サービスとして統合するためには、**B群の「Ghost Editor（ファイル経由のエージェント自動操縦）アーキテクチャ」での実装が現時点での最も現実的かつ安全な解**となります。モデルの動的変更については、ユーザーが事前にUIで強力なモデル（Gemini 3.1 Pro等）を選択しておく運用でカバーします。
...
これらのモデルは、現在のブリッジ実装においては `executeCommand` 経由で直接指定することはできません。モデルの切り替えが必要な場合は、ユーザーが IDE パネル上で事前に手動で選択する必要があります。

### モデルラベル（表示名）の取得メカニズム

ユーザーインターフェース上に表示される「Gemini 3.1 Pro」や「Claude 4.6」といったラベル（表示名）は、IDE 側のバイナリにハードコードされているのではなく、バックエンドから動的に取得されています。

- **取得フロー**:
    1. フロントエンドが `GetCascadeModelConfigs` などの RPC を呼び出す。
    2. バックエンド（Language Server）が `ClientModelConfig` メッセージを返す。
    3. このメッセージ内の `display_name` フィールドに、その時点で最適なブランド名とバージョン（例: "Gemini 3.1 Pro (High)"）が含まれている。
- **利点**: この動的な仕組みにより、Google は IDE 本体のアップデートを待つことなく、サーバー側の設定変更だけで最新モデルへのブランド改称やマイナーバージョンの更新（3.0 -> 3.1 等）をユーザーに反映させることが可能になっています。
