# Antigravity IDE 内部仕様調査レポート

> **調査日**: 2026-02-19
> **対象**: Google Antigravity IDE（VS Code フォーク）
> **目的**: 拡張機能から LLM モデルにプログラム的にアクセスする方法の特定

---

## 1. アーキテクチャ概要

Antigravity IDE は VS Code フォークであり、AI 機能を内蔵している。
AI バックエンドは **Protocol Buffers ベースの内部 RPC** で IDE ワークベンチと通信しており、
標準の `vscode.lm` 拡張 API にはモデルを登録していない。

```text
┌─────────────────────────────────────────────────┐
│  Antigravity IDE (VS Code fork)                 │
│                                                 │
│  ┌──────────┐    protobuf RPC    ┌───────────┐  │
│  │ Workbench│◄──────────────────►│ Language   │  │
│  │ (Chat UI)│  (exa.language_    │ Server     │  │
│  │          │   server_pb)       │ (Sidecar)  │  │
│  └──────────┘                    └─────┬──────┘  │
│       ▲                                │         │
│       │ commands                       │ API     │
│  ┌────┴─────┐                    ┌─────▼──────┐  │
│  │Extension │                    │ cloudcode- │  │
│  │  Host    │                    │ pa.google  │  │
│  │          │  ✗ vscode.lm 未登録 │ apis.com   │  │
│  └──────────┘                    └────────────┘  │
└─────────────────────────────────────────────────┘
```

## 2. コア拡張: `google.antigravity`

| 項目 | 値 |
|---|---|
| Extension ID | `google.antigravity` |
| 表示名 | Antigravity |
| エクスポート API | **なし** (`exports` は空) |
| `vscode.lm` 登録 | **なし** (起動時・遅延登録ともに検出されず) |
| 登録コマンド数 | 19（package.json contributes） |
| 内部コマンド総数 | 約140以上（workbench 統合含む） |

### 2.1 登録コマンド（package.json contributes）

| コマンド | 説明 |
|---|---|
| `antigravity.login` | IDE にログイン |
| `antigravity.loginWithAuthToken` | Auth Token でログイン（バックアップ） |
| `antigravity.generateCommitMessage` | コミットメッセージ生成 |
| `antigravity.restartLanguageServer` | Language Server 再起動 |
| `antigravity.copyApiKey` | API Key をクリップボードにコピー ※実行時 not found |
| `antigravity.openBrowser` | ブラウザを開く |
| `antigravity.startDemoMode` | デモモード開始 [Beta] |
| `antigravity.endDemoMode` | デモモード終了 [Beta] |
| `antigravity.import*` | VS Code / Cursor / Windsurf / Cider 設定のインポート |

### 2.2 主要な内部コマンド

#### Chat / Agent 関連

| コマンド | 動作確認結果 |
|---|---|
| `antigravity.sendPromptToAgentPanel` | ✅ プロンプト送信可能、**戻り値なし** (`undefined`) |
| `antigravity.sendTextToChat` | ❌ エラー（引数不正） |
| `antigravity.sendChatActionMessage` | ⚠️ protobuf デコードエラー発生（後述） |
| `antigravity.executeCascadeAction` | ✅ 実行可能、**戻り値なし** (`undefined`) |
| `antigravity.openAgent` | エージェントパネルを開く |
| `antigravity.agentPanel.focus` | エージェントパネルにフォーカス |
| `antigravity.startNewConversation` | 新しい会話を開始 |
| `antigravity.openInteractiveEditor` | インタラクティブエディタを開く |

#### 内部状態取得

| コマンド | 結果 |
|---|---|
| `antigravity.getDiagnostics` | ✅ 58,500文字の JSON 診断データ（後述） |
| `antigravity.getCascadePluginTemplate` | 空のプラグインテンプレートオブジェクト |
| `antigravity.getManagerTrace` | 未テスト |
| `antigravity.getWorkbenchTrace` | 未テスト |

## 3. 内部通信プロトコル

### 3.1 Protocol Buffers 使用の証拠

`sendChatActionMessage` に文字列を渡した際のエラー:

```text
cannot decode exa.language_server_pb.SendActionToChatPanelRequest from JSON:
Unexpected token 'R', "Reply with"... is not valid JSON
```

- **名前空間**: `exa.language_server_pb`
- **メッセージ型**: `SendActionToChatPanelRequest`
- **通信形式**: JSON エンコードされた Protocol Buffers

これは Google 内部の `exa` パッケージ（Code 関連ツーリング）に由来する。

### 3.2 JSON オブジェクト形式での送信

以下の形式では `undefined` が返り、エラーにならなかった（受理はされた模様）:

```json
{ "action": "sendMessage", "text": "..." }
{ "message": "...", "type": "user" }
```

ただし、いずれも **レスポンスは返らない**（UI への送信のみ）。

## 4. `getDiagnostics` の内容（抜粋）

58,500文字の JSON には以下が含まれる:

```json
{
  "isRemote": false,
  "systemInfo": {
    "operatingSystem": "linux",
    "timestamp": "2026-02-19T08:45:24.136Z",
    "userEmail": "...",
    "userName": "..."
  },
  "userSettings": {
    "lastSelectedModel": 0,
    "lastSelectedModelName": "",
    "rememberLastModelSelection": 0,
    "autocompleteSpeed": 0,
    ...
  }
}
```

**モデル関連の設定** (`lastSelectedModel`, `lastSelectedModelName`) が存在するが、
値は初期状態（`0`, `""`）。

## 5. 周辺拡張

### 5.1 Antigravity Cockpit (`jlcodes.antigravity-cockpit`)

| 項目 | 値 |
|---|---|
| エクスポート | なし |
| コマンド数 | 14 |
| 主な機能 | クォータ管理、モデルキャッシュ |

注目コマンド:

- `agCockpit.refreshModelCache` — 実行時内部エラー発生
- `agCockpit.accountTree.loadAccountQuota` — アカウントクォータ読込

### 5.2 Codex / ChatGPT (`openai.chatgpt`)

OpenAI Codex 拡張も別途インストールされている。こちらも `vscode.lm` には未登録。

## 6. `vscode.lm` API の状況

| 検証項目 | 結果 |
|---|---|
| `selectChatModels()` | 空配列（モデルなし） |
| `selectChatModels({ family: "gemini" })` | 空配列 |
| `onDidChangeChatModels` イベント | 発火なし |
| 起動後の遅延登録 | 検出されず |

## 7. 結論と推奨事項

### 現状の制約

Antigravity IDE は `vscode.lm` API にモデルを登録していないため、拡張機能から標準 API 経由で LLM にアクセスする方法はない。また、内部通信に Protobuf を使用しており、単純な JSON コマンド送信ではエラーが発生するか、戻り値が得られない。

## 8. 内部 RPC 仕様の深掘り (2026-02-19 追記)

`jetskiAgent/main.js` の解析により、内部で使用されている Protobuf メッセージの構造が判明した。

### 8.1 主要なメッセージ定義

| 名前空間 | メッセージ名 | 引数構造 (主要項目) |
|---|---|---|
| `exa.chat_client_server_pb` | `SendActionToChatPanelRequest` | `action_type`: string, `payload`: bytes (repeated) |
| `exa.language_server_pb` | `StartCascadeRequest` | `metadata`: message, `cascade_id`: string (opt), etc. |
| `exa.language_server_pb` | `GetCascadeTrajectoryRequest` | `cascade_id`: string |

**ポイント**: `payload` が `bytes` 型（Protobuf JSON 形式では Base64 エンコードが必要）であるため、通常の文字列を渡すとデコードエラーが発生する。

## 9. 「戻り値取得不可」問題へのアプローチ案

コマンド実行が `undefined` を返す問題に対し、以下の 2 段階のハックを検討する。

### アプローチ A: Trajectory (軌跡) の追跡

Jetski エージェントの回答は「軌跡」として管理されている。

1. `antigravity.getDiagnostics` から直近の `cascade_id` を抽出。
2. `exa.language_server_pb.GetCascadeTrajectoryRequest` を内部 RPC 経由で送信。
3. 返却されるバイナリ（または JSON）から LLM の回答テキストを復元する。

### アプローチ B: プロキシ/インターセプタの注入

**警告:** この「アプローチ B」で想定されている拡張機能 `extension.js` の改変および `Authorization: Bearer ...` の Auth トークンキャプチャ手法は、利用規約やセキュリティポリシーへの重大な違反となる可能性が高いです。
実験や実装を行う場合は、事前に必ず法務やコンプライアンス部門等にご確認ください。`cloudcode-pa.googleapis.com` への直接的な通信方法など、この手法に関わる実行可能な実装のガイダンスとしての利用は控えてください。

---

## 11. 物理的抽出試行とフォレンジック調査結果 (2026-02-20)

プログラム的に回答を直接取得できない問題に対し、クリップボードやファイルシステムを介した「外部からの抽出」を試行した。

### 11.1 クリップボード経由の抽出テスト

- **手法**: `antigravity.agentPanel.focus` でパネルにフォーカスを当てた後、VS Code および OS レベルのコピーコマンドを発行。
- **検証用プロンプト**: 「Say hello and repeat the word BANANA exactly once.」
- **結果**: ❌ **失敗**
  - `workbench.action.chat.copyAll` 等の標準チャットコマンドは Agent Panel（Webview）に対しては機能せず、`command not found` または無反応。
  - `xdotool` による OS レベルの `Ctrl+A` -> `Ctrl+C` シミュレーションも失敗。
- **原因の特定**:
  - デスクトップ環境が **Wayland** (`XDG_SESSION_TYPE=wayland`) であるため、伝統的な X11 ベースの自動化ツール（`xdotool`）がウィンドウ間の操作やキー送信を制限されている。
  - Webview 内のテキストがデフォルトで選択状態にならないため、単純なコピーコマンドでは内容を取得できない。

### 11.2 ファイルシステムおよびログのフォレンジック調査

- **手法**: `~/.config/Antigravity/` および `~/.gemini/` 以下のすべてのファイルから、検証用キーワード 「BANANA」 を検索。
- **検索対象**: SQLite (`*.vscdb`), LevelDB (`*.leveldb`, `*.log`), キャッシュファイル, 拡張機能ログ。
- **結果**: ❌ **直接的なログの欠如**
  - `state.vscdb` (Global Storage) にはターミナル履歴などは残るが、**Agent Panel の会話履歴は保存されていない**。
  - `conversations/*.pb` ファイルは存在するが、直近の会話内容がプレーンテキストで含まれている形跡はない。
  - `Antigravity.log` には、Gemini モデルへのリクエスト時に容量不足（503 Unavailable）が発生した際のデバッグログは記録されていたが、成功時のレスポンスボディは記録されない。
- **結論**: 会話履歴はメモリ上、もしくは Google のクラウド側にのみ保持されており、ローカルのデータベースファイル等には（少なくとも解析容易な形では）永続化されていない。

### 11.3 ネットワークおよび IPC 調査

- **手法**: `netstat` および `curl` による内部ポートの走査。
- **結果**:
  - `antigravity` プロセスが `46741` 等の高位ポートで LISTEN していることを確認。
  - ただし、`/json/list` (CDP) 等の標準的なデバッグエンドポイントは有効化されていないか、アクセスが制限されている。

## 12. 最終的な結論と今後の展望 (現時点)

### 現状の到達点

1. **API 経由**: `vscode.lm` は未登録。内部コマンド `sendPromptToAgentPanel` は「送信」専用であり「受信」用のフックがない。
2. **ファイル経由**: ローカルディスクに会話ログは残らない。
3. **物理操作経由**: Wayland 環境の制限により、外部からのキー操作シミュレーションによる抽出は極めて困難。

### 今後の可能性

- **Wayland 自動化の深掘り**: `ydotool` (uinput 経由) や、Compositor 固有の自動化 API を使用して、物理的に Webview 内を全選択・コピーする。
- **Trajectory API の再検証**: 以前失敗した `GetCascadeTrajectoryRequest` を、より正確な Protobuf バイナリ構造で構築して再試行する。
- **カスタム MCP サーバの活用**: 逆に「AI から MCP サーバに対して回答を書き込ませる」という逆方向の通信経路（Exfiltration）が、現在最も確実にプログラムから回答を回収できるルートである。

---

## 13. 過去の検証記録：同期取得ハック（v1.3）の失敗と教訓

プログラムから同期的にLLM応答を取得するため、一時的に以下のハック手法を実装・検証したが、実用性と安定性の観点から破棄された。

### 試行されたアプローチ (SQLite連携 + Trace API)

1. **`cascade_id` の捕捉**: Antigravityが実行する複雑なタスク（Cascadeイベント）の進行状況は、VS Code内部のSQLiteデータベース（`state.vscdb`）の `antigravity.cascade.state` 等のキーに保存されることを特定。
2. **データベースのポーリング**: 拡張機能側から `state.vscdb` を定期的に読み込み、新しい `cascade_id` の発行や状態変化（完了状態への遷移）を検知。
3. **Trace API による取得**: 完了検知後、内部API `antigravity.getTrace(cascade_id)` を呼び出し、LLMの軌跡（Trajectory）データから生成されたテキストを抽出する。

### 破棄された理由（限界）

- **対象の限定性**: この手法は「エディタ上での複雑なコード生成（Cascadeイベント）」にのみ有効であり、エージェントパネルへの「単純なチャットプロンプト（`sendPromptToAgentPanel`）」では `cascade_id` が全く生成されないか、データベースに状態が記録されないため、取得不可能であった。
- **アーキテクチャの脆弱性**: IDEのローカルデータベース（通常は拡張機能が直接読み書きするべきではない内部ストア）のスキーマや動作タイミングに強く依存しており、Antigravity拡張自体のマイナーアップデートで即座に動作しなくなるリスクが極めて高かった。
- **非同期処理の複雑化**: DBのファイルロック競合回避や、不明瞭な完了タイミングのポーリングにより、ブリッジ側のコードが肥大化・不安定化していた。

この失敗の教訓から、内部状態を追跡するアプローチは完全に破棄され、後述の「AI自身にファイルを出力させる（Stigmergyパターン）」というクリーンな設計に回帰した。

---

## 14. 実装上の推奨事項（ブリッジ構成案）

Antigravity 会話の「自動抽出」を諦め、**「AI に指示して、特定のファイルや MCP ツールに結果を出力させる」** 運用に切り替えることを推奨する。
これにより、UI や非公開 API のハックに頼ることなく、確実なデータ連携が可能になる。

### 具体的なワークフロー（The Stigmergy Pattern）

1. **タスクの委譲 (Fire-and-Forget)**
   - OpenCode 等の外部 AI クライアントから、`dispatch_agent_task` ツールを使用して Antigravity（Gemini 3 Pro）にタスクを指示する。
   - 指示には必ず「完了後に特定のファイル（例: `DONE.md` や `ARCHITECTURE.md`）に結果を出力すること」を含める。
2. **非同期の実行と出力**
   - Antigravity は指示を受け取り、バックグラウンドで自律的に思考・調査・コード生成を行う。
   - 完了後、指示通りにファイルシステムに結果を書き込む。
3. **結果の回収 (Polling/Notification)**
   - 外部 AI クライアントは、ファイルシステムを監視するか、一定時間後に `read_file` ツールを使用して指定したファイルを読み込む。
   - ファイルの内容から Antigravity の思考プロセスや生成されたコード・設計ドキュメントを取得し、次のアクションに繋げる。

### 今後の開発方針

- **安定性の重視**: 内部データベース（vscdb）のポーリングや難読化された RPC プロトコルのハックは、IDE のバージョンアップによって容易に破壊されるリスクが高いため避ける。
- **ファイルシステムを介した協調**: AI エージェント同士の通信チャネルとして、最も堅牢で標準的なファイルシステムを採用する（Stigmergy アプローチ）。
- **デバッグ情報の充実**: `dispatch_agent_task` 等の一方通行のコマンド実行において、失敗時に可能な限り詳細なエラーを Extension 側の出力パネル（Output Channel）に残す仕組みを維持する。
